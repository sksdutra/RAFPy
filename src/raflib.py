...
class RAFLib:
    def __init__(self, path_raf, path_raf_data):
        try:
            ...
            print error
            exit()
        self.le_data = lambda data: unpack("<I", data)[0]  # Interpret data in little-endian order
        # RAF Check
        ...
        if self.magic_number != 0x18be0ef0:
            ...
            self.path_list_offset = self.le_data(self.raf.read(4))  # Offset where's stored path info
    ...
    def __get_file_entries(self):
        ...
        file_entries_list = [[self.le_data(self.raf.read(4)) for file_entry in range(4)]
                             for file_entries in range(number_of_entries)]
    ...
    def unpack_content(self):
        ...
        file_entries = sorted(self.__get_file_entries(), key=lambda index: index[3])
        path_list_entries = self.__get_path_list_entries()
        print "[Info]Generating %s files." % len(path_list_entries)
        # Creating a list of paths to unpack files
        path_list = [self.raf.read(size) for relative_offset, size in path_list_entries]
        if len(path_list) == len(file_entries):
            for entry in range(len(path_list)):
                ...
                if not the_path == "" and not path.exists(the_path):
                    makedirs(the_path)
                ...
                # Trying to decompress using zlib and write in correct file
                try:
                    actual_file.write(decompress(raf_data))
                except:
                    actual_file.write(raf_data)  # If can't decompress write as is
        else:
